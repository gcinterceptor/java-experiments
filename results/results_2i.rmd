...
```{r}
al.gci.warm <- accesslog("2i", "gci_T160_Java9_YoungGCOnly", 1)
al.nogci.warm <- accesslog("2i", "nogci_T160_Java9_YoungGCOnly", 1)
...
```{r}
failed.requests.gci <- filter(al.gci.warm, status == 503)
...
failed.requests.nogci <- filter(al.nogci.warm, status == 503)
