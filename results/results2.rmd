...
* N service instances (VM with 2 cores and 1GB RAM each). JVM heap set to 512MB.
...
* Constants:
     ...
     * Load (Constant and equals to N\*40 QPS) The system was less than 50% utiilzation. The goal is to minimize the effect of context switching and environment sources of noise (we would like to focus on the impact of the GC on the request time)
...
```{r}
gci.1i.nostate <- accesslog("1i", "gci_T40_M262144_W0", REP, WARMUP_SECS, DURATION_SECS)
gci.1i.state <- accesslog("1i", "gci_T40_M262144_W256", REP, WARMUP_SECS, DURATION_SECS)
gci.2i.nostate <- accesslog("2i", "gci_T80_M262144_W0", REP, WARMUP_SECS, DURATION_SECS)
gci.2i.state <- accesslog("2i", "gci_T80_M262144_W256", REP, WARMUP_SECS, DURATION_SECS)
...
nogci.2i.nostate <- accesslog("2i", "nogci_T80_M262144_W0", REP, WARMUP_SECS, DURATION_SECS)
nogci.2i.state <- accesslog("2i", "nogci_T80_M262144_W256", REP, WARMUP_SECS, DURATION_SECS)
nogci.1i.nostate <- accesslog("1i", "nogci_T40_M262144_W0", REP, WARMUP_SECS, DURATION_SECS)
nogci.1i.state <- accesslog("1i", "nogci_T40_M262144_W256", REP, WARMUP_SECS, DURATION_SECS)
...
```{r}
sd.imp.1i.table <- as.table(matrix(c(
...
colnames(sd.imp.1i.table) <- c("Latency Stdev Improvement (%)")
rownames(sd.imp.1i.table) <- c(
...
```{r}
sd.imp.2i.table <- as.table(matrix(c(
...
colnames(sd.imp.2i.table) <- c("Latency Stdev Improvement (%)")
rownames(sd.imp.2i.table) <- c(
 "Stateless & High Heap Pressure",
 "Statefull & High Heap Pressure"
)
sd.imp.2i.table
